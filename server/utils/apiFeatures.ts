...
export class APIFeatures {
	...
	filter() {
		const queryObj: Query<Object, any> = { ...this.queryString };
		...
		excludedFields.forEach((curr: string) => delete queryObj[curr]);
		...
		queryStr = queryStr.replace(/\b(gte|gt|lte|lt)\b/g, (matchingStr: string) => `$${matchingStr}`);
